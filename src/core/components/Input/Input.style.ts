...
export const getInputStyles = (isError: boolean, className: string) => ({
  ...
  component_input_wrapper: clsx(
    ...
    isError
      ? 'border-red-600 bg-input-background-error focus-within:border-red-600'
      : 'border-input-border bg-input-background focus-within:border-indigo-400 focus-within:bg-input-focus',
    className,
  ),
  input: clsx(
    'w-full appearance-none border-none bg-transparent outline-none placeholder:text-gray-400 text-14',
    isError ? 'text-red-600' : 'text-input-text focus:text-indigo-400',
  ),
